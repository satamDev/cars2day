...
jQuery(document).ready(function($){
  ...
  cssEase: 'linear'
    ...
    $('.backbnrtab .nav-link').click(function() {
        setTimeout(function () {
             $('.productslide').slick('refresh');
            $('.backbnrtab').addClass('active');
         }, 2000);
...
window.addEventListener("scroll", reveal);
    ...
    var principle = 1000000, month = 24, interest = 9.5;    
...
function calculate_interest(){    
    inst = (principle * (parseFloat(interest) * 0.01));
    inst = inst / month;
    // total = ((principle / parseInt(month)) + inst);

    var total_inst = inst * month;
    // console.log(inst);

    $(".emi-res").text(inst.toFixed(2));
    $(".emi_month_to_year").text(month/12);

    $(".td_month").text(month);
    $(".td_principle").text(principle.toLocaleString());
    $(".td_interest").text(total_inst.toFixed(2).toLocaleString());
    $(".td_total").text((principle + (parseFloat(inst)*month)).toFixed(2).toLocaleString());
