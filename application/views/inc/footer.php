<?php
defined('BASEPATH') OR exit('No direct script access allowed');
?>

<footer class="footer pt-5 pb-4">
        <div class="container">
            <div class="ftrtop">
                <div class="d-flex align-items-center">
                    <a href="<?=base_url()?>" class="ftrlgo mr-3">
                        <img src="<?=base_url()?>assets/images/logo.png" alt="logo" width="150">
                    </a>
                    <a href="" class="soclink mr-3"><i class="fab fa-facebook-f fa-fw"></i></a>
                    <a href="" class="soclink mr-3"><i class="fab fa-twitter fa-fw"></i></a>
                    <a href="" class="soclink mr-3"><i class="fab fa-instagram fa-fw"></i></a>
                    <a href="" class="soclink mr-3"><i class="fab fa-youtube fa-fw"></i></a>
                    <a href="tel:044-1234 5678" class="ftrph ml-auto">
                        <svg fill="#fff" height="24" width="24" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                             viewBox="0 0 473.806 473.806" style="enable-background:new 0 0 473.806 473.806;" xml:space="preserve">
                                <path d="M374.456,293.506c-9.7-10.1-21.4-15.5-33.8-15.5c-12.3,0-24.1,5.3-34.2,15.4l-31.6,31.5c-2.6-1.4-5.2-2.7-7.7-4
                                    c-3.6-1.8-7-3.5-9.9-5.3c-29.6-18.8-56.5-43.3-82.3-75c-12.5-15.8-20.9-29.1-27-42.6c8.2-7.5,15.8-15.3,23.2-22.8
                                    c2.8-2.8,5.6-5.7,8.4-8.5c21-21,21-48.2,0-69.2l-27.3-27.3c-3.1-3.1-6.3-6.3-9.3-9.5c-6-6.2-12.3-12.6-18.8-18.6
                                    c-9.7-9.6-21.3-14.7-33.5-14.7s-24,5.1-34,14.7c-0.1,0.1-0.1,0.1-0.2,0.2l-34,34.3c-12.8,12.8-20.1,28.4-21.7,46.5
                                    c-2.4,29.2,6.2,56.4,12.8,74.2c16.2,43.7,40.4,84.2,76.5,127.6c43.8,52.3,96.5,93.6,156.7,122.7c23,10.9,53.7,23.8,88,26
                                    c2.1,0.1,4.3,0.2,6.3,0.2c23.1,0,42.5-8.3,57.7-24.8c0.1-0.2,0.3-0.3,0.4-0.5c5.2-6.3,11.2-12,17.5-18.1c4.3-4.1,8.7-8.4,13-12.9
                                    c9.9-10.3,15.1-22.3,15.1-34.6c0-12.4-5.3-24.3-15.4-34.3L374.456,293.506z M410.256,398.806
                                    C410.156,398.806,410.156,398.906,410.256,398.806c-3.9,4.2-7.9,8-12.2,12.2c-6.5,6.2-13.1,12.7-19.3,20
                                    c-10.1,10.8-22,15.9-37.6,15.9c-1.5,0-3.1,0-4.6-0.1c-29.7-1.9-57.3-13.5-78-23.4c-56.6-27.4-106.3-66.3-147.6-115.6
                                    c-34.1-41.1-56.9-79.1-72-119.9c-9.3-24.9-12.7-44.3-11.2-62.6c1-11.7,5.5-21.4,13.8-29.7l34.1-34.1c4.9-4.6,10.1-7.1,15.2-7.1
                                    c6.3,0,11.4,3.8,14.6,7c0.1,0.1,0.2,0.2,0.3,0.3c6.1,5.7,11.9,11.6,18,17.9c3.1,3.2,6.3,6.4,9.5,9.7l27.3,27.3
                                    c10.6,10.6,10.6,20.4,0,31c-2.9,2.9-5.7,5.8-8.6,8.6c-8.4,8.6-16.4,16.6-25.1,24.4c-0.2,0.2-0.4,0.3-0.5,0.5
                                    c-8.6,8.6-7,17-5.2,22.7c0.1,0.3,0.2,0.6,0.3,0.9c7.1,17.2,17.1,33.4,32.3,52.7l0.1,0.1c27.6,34,56.7,60.5,88.8,80.8
                                    c4.1,2.6,8.3,4.7,12.3,6.7c3.6,1.8,7,3.5,9.9,5.3c0.4,0.2,0.8,0.5,1.2,0.7c3.4,1.7,6.6,2.5,9.9,2.5c8.3,0,13.5-5.2,15.2-6.9
                                    l34.2-34.2c3.4-3.4,8.8-7.5,15.1-7.5c6.2,0,11.3,3.9,14.4,7.3c0.1,0.1,0.1,0.1,0.2,0.2l55.1,55.1
                                    C420.456,377.706,420.456,388.206,410.256,398.806z"/>
                                <path d="M256.056,112.706c26.2,4.4,50,16.8,69,35.8s31.3,42.8,35.8,69c1.1,6.6,6.8,11.2,13.3,11.2c0.8,0,1.5-0.1,2.3-0.2
                                    c7.4-1.2,12.3-8.2,11.1-15.6c-5.4-31.7-20.4-60.6-43.3-83.5s-51.8-37.9-83.5-43.3c-7.4-1.2-14.3,3.7-15.6,11
                                    S248.656,111.506,256.056,112.706z"/>
                                <path d="M473.256,209.006c-8.9-52.2-33.5-99.7-71.3-137.5s-85.3-62.4-137.5-71.3c-7.3-1.3-14.2,3.7-15.5,11
                                    c-1.2,7.4,3.7,14.3,11.1,15.6c46.6,7.9,89.1,30,122.9,63.7c33.8,33.8,55.8,76.3,63.7,122.9c1.1,6.6,6.8,11.2,13.3,11.2
                                    c0.8,0,1.5-0.1,2.3-0.2C469.556,223.306,474.556,216.306,473.256,209.006z"/>
                        </svg>
                        <span class="ml-2">044 - 1234 5678</span></a>
                </div>
            </div>
            <div class="ftrmid pt-5">
                <div class="row">
                    <div class="col-lg-2 col-md-4">
                        <h3 class="mb-3 text-white">Store</h3>
                        <ul class="ftrmnu">
                            <li><a href="">Browse by Category</a></li>
                            <li><a href="">Premium Cars</a></li>
                        </ul>
                    </div>
                    <div class="col-lg-2 col-md-4">
                        <h3 class="mb-3 text-white">Sell</h3>
                        <ul class="ftrmnu">
                            <li><a href="<?=base_url()?>sell">Schedule Free Evaluation</a></li>
                        </ul>
                    </div>
                    <div class="col-lg-2 col-md-4">
                        <h3 class="mb-3 text-white">Finance</h3>
                        <ul class="ftrmnu">
                            <li><a href="<?=base_url()?>finance/emi_calculator">EMI Calculator</a></li>
                            <li><a href="">Cars2day Loans</a></li>
                        </ul>
                    </div>
                    <div class="col-lg-2 col-md-4">
                        <h3 class="mb-3 text-white">About</h3>
                        <ul class="ftrmnu">
                            <li><a href="">Management</a></li>
                            <li><a href="">Contact Us</a></li>
                            <li><a href="">CSR</a></li>
                            <li><a href="">Investors</a></li>
                        </ul>
                    </div>
                    <div class="col-lg-2 col-md-4">
                        <h3 class="mb-3 text-white">Careers</h3>
                        <ul class="ftrmnu">
                            <li><a href="">Open Position</a></li>
                            <li><a href="">Post Your Resume</a></li>
                        </ul>
                    </div>
                    <div class="col-lg-2 col-md-4">
                        <h3 class="mb-3 text-white">More</h3>
                        <ul class="ftrmnu">
                            <li><a href="<?=base_url()?>carvenience">Carvenience Service</a></li>
                            <li><a href="<?=base_url()?>autoteilee">Autoteilee Parts</a></li>
                            <li><a href="">FAQ</a></li>
                            <li><a href="<?=base_url()?>article">Research Articles</a></li>
                        </ul>
                    </div>
                </div>
            </div>
            
            
            <div class="ftrbtm text-center text-white">
                <ul class="ftrlink mb-4">
                    <li><a href="">Privacy Policy</a></li> |  <li><a href="">Terms Of Use</a></li> | <li><a href="">Cookie Policy</a></li> | <li><a href="">Feedback</a></li> | <li><a href="">Partner With Us</a></li> | <li><a href="">Reach to Founder</a></li>
                </ul>
                <small class="d-block">Copy right – Better Drive Technology Pvt Ltd </small>
            </div>
            
        </div>
</footer>

<a href="#" id="toTopBtn" class="" data-abc="true">
    <span>
       <svg width="24" height="24" fill="#e74141" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
             viewBox="0 0 384.923 384.923" style="enable-background:new 0 0 384.923 384.923;" xml:space="preserve">
            <path id="Arrow_Upward" d="M321.337,122.567L201.046,3.479c-4.776-4.728-12.391-4.547-17.179,0l-120.291,119.1
                c-4.74,4.704-4.74,12.319,0,17.011c4.752,4.704,12.439,4.704,17.191,0l99.551-98.552v331.856c0,6.641,5.438,12.03,12.151,12.03
                s12.151-5.39,12.151-12.03V41.025l99.551,98.552c4.74,4.704,12.439,4.704,17.179,0C326.089,134.886,326.089,127.27,321.337,122.567
                z"/>
        </svg>
    </span>
</a>
<a href="#" class="btnchat">
    <div class="d-flex align-items-center">
        <svg height="40" width="40" fill="#e74141" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
           viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                <path d="M452.923,34.133H59.077C26.502,34.133,0,60.635,0,93.21v228.431c0,32.575,26.502,59.077,59.077,59.077h8.097l-23.33,80.36
                    c-1.594,5.49,0.547,11.38,5.296,14.564c2.219,1.487,4.767,2.224,7.31,2.224c2.899,0,5.791-0.958,8.174-2.854l118.542-94.295
                    h269.757c32.575,0,59.077-26.502,59.077-59.077V93.21C512,60.635,485.498,34.133,452.923,34.133z M485.744,321.641
                    c0,18.097-14.723,32.82-32.82,32.82H178.58c-2.968,0-5.85,1.006-8.172,2.854l-89.849,71.471l16.704-57.537
                    c1.151-3.966,0.37-8.242-2.11-11.544c-2.48-3.302-6.368-5.245-10.497-5.245H59.077c-18.097,0-32.82-14.723-32.82-32.82V93.21
                    c0-18.097,14.723-32.82,32.82-32.82h393.846c18.097,0,32.82,14.723,32.82,32.82V321.641z"/>
                <path d="M131.282,164.759c-27.146,0-49.231,22.084-49.231,49.231s22.084,49.231,49.231,49.231
                    c27.147,0,49.231-22.084,49.231-49.231S158.429,164.759,131.282,164.759z M131.282,236.964c-12.667,0-22.974-10.307-22.974-22.974
                    s10.307-22.974,22.974-22.974s22.974,10.307,22.974,22.974S143.949,236.964,131.282,236.964z"/>
                <path d="M256,164.759c-27.146,0-49.231,22.084-49.231,49.231s22.084,49.231,49.231,49.231s49.231-22.084,49.231-49.231
                    S283.146,164.759,256,164.759z M256,236.964c-12.667,0-22.974-10.307-22.974-22.974s10.307-22.974,22.974-22.974
                    s22.974,10.307,22.974,22.974S268.667,236.964,256,236.964z"/>
                <path d="M380.718,164.759c-27.146,0-49.231,22.084-49.231,49.231s22.084,49.231,49.231,49.231
                    c27.147,0,49.231-22.084,49.231-49.231S407.865,164.759,380.718,164.759z M380.718,236.964c-12.667,0-22.974-10.307-22.974-22.974
                    s10.307-22.974,22.974-22.974c12.667,0,22.974,10.307,22.974,22.974S393.385,236.964,380.718,236.964z"/>
        </svg>
        <span class="pl-2">Chat</span>
    </div>
</a>